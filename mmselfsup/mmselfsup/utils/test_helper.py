...
####################################################################
from .collect import dist_forward_collect_Rui
...
def multi_gpu_test_Rui(model, data_loader, tmpdir=None, gpu_collect=False):
    model.eval()
    ...
    rank, world_size = get_dist_info()
    results = dist_forward_collect_Rui(test_mode_func, data_loader, rank,
                                   len(data_loader.dataset))
    return results
