...
def get_samples_tiny(root, folder_to_idx, extensions):
    ...
    root = osp.expanduser(root)
    for folder_name in sorted(list(folder_to_idx.keys())):
        _dir = osp.join(root, folder_name, 'images')
        for _, _, fns in sorted(os.walk(_dir)):
            for fn in sorted(fns):
                if has_file_allowed_extension(fn, extensions):
                    path = osp.join(folder_name, 'images', fn)
                    item = (path, folder_to_idx[folder_name])
                    samples.append(item)
    return samples
###############################################################################
...
class ImageNet(BaseDataSource):
    ...
    def load_annotations(self):
        if self.ann_file is None:
            ...
            # print(folder_to_idx,'2222222222222222222222222\n')
            # breakpoint()
            # change it to get_samples_tiny for tiny-imagenet ##############
            samples = get_samples_tiny(
            # samples = get_samples(
                self.data_prefix,
                folder_to_idx,
            ...
            with open(self.ann_file) as f:
                samples = [x.strip().rsplit(' ', 1) for x in f.readlines()] ########################### change from ' ' to '\t'
        else:
