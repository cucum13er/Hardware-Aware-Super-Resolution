...
class SimCLR_Multidevice_cls(BaseModel):
    """SimCLR.
    ...
    def __init__(self, backbone, neck=None, head=None, init_cfg=None):
        super(SimCLR_Multidevice_cls, self).__init__(init_cfg)
    ...
    def forward_test(self, img, label, **kwargs):
        with torch.no_grad():
            ...
            cls_score, _ = self.head(x,label_all) # cross entropy loss
            # x, features = self.head(x,label_all)
            # breakpoint()
            results = dict()
            results.update(zip(cls_score[0].cpu(), label_all.cpu())) 
            # print(results)
        return results
    ...
    def forward_train(self, img, label, **kwargs):#####################add label
        ...
        x, _ = self.head(x,label_all) # cross entropy loss
        losses = self.head.loss(x, label_all)
        # print(losses, '111111111111111\n')
        return losses
"""
pos_ind:
