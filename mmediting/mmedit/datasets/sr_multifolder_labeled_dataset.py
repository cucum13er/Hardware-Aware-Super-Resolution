...
class SRMultiFolderLabeledDataset(BaseSRDataset):
    ...
    def load_annotations(self):
        ...
        data_infos = []
        # lq_paths = []
        folder_to_idx = find_folders(self.lq_root)
        ...
        for i, (filename, gt_label) in enumerate(samples):
            ...
            ##############
            info = {'lq_root': self.lq_root}
            info['lq_path'] = osp.join(self.lq_root, filename)
            info['gt_label'] = np.array(gt_label, dtype=np.int64)
            info['idx'] = int(i)
            info['gt_path'] = osp.join(self.gt_folder, osp.basename(filename))
