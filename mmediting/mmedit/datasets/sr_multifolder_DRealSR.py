...
class SRDRealSR(BaseSRDataset):
    ...
    def load_annotations(self):
        ...
        for i, (filename, gt_label) in enumerate(samples):
            ...
            if self.scale == 4:
                prefix, fn = osp.split(gt_name)
                gt_fn = fn.replace("x1", "x4")
                gt_name = osp.join(prefix, gt_fn)
            elif self.scale == 2:
                if "ARC" in gt_name:
                    prefix, fn = osp.split(gt_name)
                    lq, ext = osp.splitext(fn)
                    gt_fn = lq + "_gt"
                    gt_name = osp.join(prefix, gt_fn+ext)
                else:
                    prefix, fn = osp.split(gt_name)
                    gt_fn = fn.replace("x1", "x2")
                    gt_name = osp.join(prefix, gt_fn)
            else:
                raise ValueError('Scale factor invalid!')
        ...
        # breakpoint()
        '''
        data_infos is like that:
            
        {'root': '/media/rui/Rui_T7/DRealSR/Train_x4/train_LR', 
         'lq_path': '/media/rui/Rui_T7/DRealSR/Train_x4/train_LR/sony/sony_201_x1_53.png', 
         'gt_label': array(4), 
         'idx': 30501, 
         'gt_path': '/media/rui/Rui_T7/DRealSR/Train_x4/train_HR/sony/sony_201_x4_53.png'}
