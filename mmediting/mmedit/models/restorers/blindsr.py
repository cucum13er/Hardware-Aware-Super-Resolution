...
class BlindSR(BaseModel):
    ...
    #     z = self.neck(contrastive_feat)[0]
    #     z = z / (torch.norm(z, p=2, dim=1, keepdim=True) + 1e-10)
    #     z = torch.cat(GatherLayer.apply(z), dim=0)  # (2N)xd
    #     s = torch.matmul(z, z.permute(1, 0))  # (2N)x(2N)
    #     # print(s)
    #     # breakpoint()
    #     loss_contrastive = self.contrastive_loss(s, gt_label)
    #     losses['loss_contrastive'] = loss_contrastive
        
    #     outputs = dict(
    #         losses=losses,
    #         num_samples=len(gt.data),
    #         results=dict(lq=lq.cpu(), gt=gt.cpu(), output=sr_image.cpu())
    #         )
    #     # breakpoint()
    #     return outputs
